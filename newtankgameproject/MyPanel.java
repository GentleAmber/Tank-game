...
public class MyPanel extends JPanel implements Runnable, KeyListener {
    ...
    public void paint(Graphics g) {//Graphics is like the painting tool
        ...
        if (myTank.getStatus() == Tank.Status.DEAD) {
            Graphics2D gGO = (Graphics2D) g;
            gGO.setColor(Color.WHITE);
            gGO.setFont(new Font("Arial", Font.BOLD, 108));
            gGO.drawString("Game Over!", 200, 350);
        }
